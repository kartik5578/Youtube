...
const FilterSidebar = () => {
  return (
    <div className=" text-white w-full rounded-lg spsce-y-2">
      <div className="mb-4 flex items-center text-white space-x-3 border border-[#02C173] rounded-lg p-1">
      ...
      <div className="mb-4 mt-2">
        ...
        <div className="flex flex-wrap gap-2">
          {['Education', 'Entertainment', 'Gaming', 'Infotainment', 'Travel', 'Tech', 'Sports', 'News', 'Fashion & Beauty', 'Music'].map(niche => (
            <button
              key={niche}
              className="px-3 py-1 bg-transparent border border-[#02C173] rounded-full hover:bg-[#02C173] "
            >
              {niche}
            </button>
          ))}
      ...
      <div className="mb-4 mt-2">
        ...
        <div className="flex flex-wrap gap-2">
          {['< 5K', '5K - 10K', '10K - 25K', '25K - 50K', '50K - 100K', '100K - 200K', '200K - 500K', '500K - 1M', '1M - 2M', '2M - 5M', '5M - 10M', '> 10M'].map(subs => (
            <button
              key={subs}
              className="px-3 py-1 bg-transparent border border-[#02C173] rounded-full hover:bg-green-700"
            >
              {subs}
            </button>
          ))}
      ...
      <div className="mb-4 mt-2">
        ...
        <span className="ml-2 mb-2">1K - 10M+</span>
          <input type="range" min="1" max="10" className="w-full h-1 bg-[#02C173]" />
        </div>
      </div>
    </div>
  );
